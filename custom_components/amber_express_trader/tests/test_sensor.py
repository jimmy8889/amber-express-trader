...
from custom_components.amber_express_trader import AmberRuntimeData, SiteRuntimeData
...
from custom_components.amber_express_trader.sensor import (
    CHANNEL_PRICE_TRANSLATION_KEY,
    SENSOR_DESCRIPTIONS,
    AmberBestChargeWindowSensor,
    AmberBestExportWindowSensor,
    AmberForecastHorizonSensor,
    AmberNextNegativePriceSensor,
    AmberNextSpikeSensor,
    AmberPriceSensor,
    AmberSensor,
    AmberSensorDescription,
    AmberTradingActionSensor,
    async_setup_entry,
)
from custom_components.amber_express_trader.utils import get_http_status_label

TRADING_SENSOR_COUNT = 6
...
class TestAmberPriceSensor:
    ...
    def test_price_sensor_trading_attributes_keep_existing_forecasts(
        self,
        mock_coordinator_with_data: MagicMock,
        mock_config_entry: MockConfigEntry,
        mock_subentry: MagicMock,
    ) -> None:
        """Price attributes retain existing forecasts and add trading helpers."""
        sensor = AmberPriceSensor(
            coordinator=mock_coordinator_with_data,
            entry=mock_config_entry,
            subentry=mock_subentry,
        ...
        assert ATTR_FORECAST in attrs
        assert ATTR_DETAILED_FORECAST in attrs
        assert attrs["raw_price"] == 0.25
        assert attrs["effective_price"] == 0.25
        assert attrs["is_effectively_zero"] is False
        assert attrs["is_export_profitable"] is False
        assert attrs["is_grid_charge_profitable"] is False
        assert attrs["is_spike"] is False
        assert attrs["chartForecast"] == [
            {"x": "2024-01-01T10:05:00+00:00", "y": 0.28},
            {"x": "2024-01-01T10:10:00+00:00", "y": 0.29},
        ]
        assert attrs["trading_action"] == "export_to_grid"

    def test_price_sensor_feed_in_sign_handling_remains_consistent(
        self,
        mock_coordinator_with_data: MagicMock,
        mock_config_entry: MockConfigEntry,
        mock_subentry: MagicMock,
    ) -> None:
        """Feed-in price sign handling remains unchanged."""
        sensor = AmberPriceSensor(
            coordinator=mock_coordinator_with_data,
            entry=mock_config_entry,
            subentry=mock_subentry,
        ...
        assert sensor.native_value == -0.10
        attrs = sensor.extra_state_attributes
        assert attrs["raw_price"] == -0.10
        assert attrs["is_export_profitable"] is True
        assert attrs["chartForecast"][0]["y"] == -0.12
...
class TestAmberTradingSensors:
    """Tests for trading helper sensors."""

    def test_best_export_window_sensor(
        self,
        mock_coordinator_with_data: MagicMock,
        mock_config_entry: MockConfigEntry,
        mock_subentry: MagicMock,
    ) -> None:
        """Best export window exposes clear window attributes."""
        sensor = AmberBestExportWindowSensor(
            coordinator=mock_coordinator_with_data,
            entry=mock_config_entry,
            subentry=mock_subentry,
        )
        ...
        attrs = sensor.extra_state_attributes
        assert attrs["start_time"] == "2024-01-01T10:05:00+00:00"
        assert attrs["average_price"] == 0.12
        assert attrs["interval_count"] == 1
        assert attrs["reason"] == "export_price_above_floor"

    def test_best_charge_window_sensor(
        self,
        mock_coordinator_with_data: MagicMock,
        mock_config_entry: MockConfigEntry,
        mock_subentry: MagicMock,
    ) -> None:
        """Best charge window exposes clear window attributes."""
        sensor = AmberBestChargeWindowSensor(
            coordinator=mock_coordinator_with_data,
            entry=mock_config_entry,
            subentry=mock_subentry,
        )
        ...
        assert sensor.native_value == 0.28
        assert sensor.extra_state_attributes["reason"] == "lowest_import_price"

    def test_next_spike_and_negative_price_sensors_return_none_without_match(
        self,
        mock_coordinator_with_data: MagicMock,
        mock_config_entry: MockConfigEntry,
        mock_subentry: MagicMock,
    ) -> None:
        """Next spike and negative price sensors return empty data without a match."""
        spike = AmberNextSpikeSensor(
            coordinator=mock_coordinator_with_data,
            entry=mock_config_entry,
            subentry=mock_subentry,
        )
        negative = AmberNextNegativePriceSensor(
            coordinator=mock_coordinator_with_data,
            entry=mock_config_entry,
            subentry=mock_subentry,
        )

        assert spike.native_value is None
        assert spike.extra_state_attributes == {}
        assert negative.native_value is None
        assert negative.extra_state_attributes == {}

    def test_trading_action_sensor(
        self,
        mock_coordinator_with_data: MagicMock,
        mock_config_entry: MockConfigEntry,
        mock_subentry: MagicMock,
    ) -> None:
        """Trading action sensor returns the current recommendation."""
        sensor = AmberTradingActionSensor(
            coordinator=mock_coordinator_with_data,
            entry=mock_config_entry,
            subentry=mock_subentry,
        )

        assert sensor.native_value == "export_to_grid"
        assert sensor.extra_state_attributes["reason"] == "current_price_thresholds"
...
class TestAsyncSetupEntry:
    ...
    async def test_setup_entry_creates_sensors(
        ...
        # 2 channels x 1 price sensor + description-driven sensors + forecast_horizon + trading sensors
        assert len(added_entities) == 2 + len(SENSOR_DESCRIPTIONS) + 1 + TRADING_SENSOR_COUNT
    ...
    async def test_setup_entry_uses_site_channels(
        ...
        # 1 channel x 1 price sensor + description-driven sensors + forecast_horizon + trading sensors
        assert len(added_entities) == 1 + len(SENSOR_DESCRIPTIONS) + 1 + TRADING_SENSOR_COUNT
...
class TestAmberPollingStatsSensor:
    ...
    def test_polling_stats_sensor_native_value_with_observation(
        ...
        from custom_components.amber_express_trader.polling import CDFPollingStats  # noqa: PLC0415
