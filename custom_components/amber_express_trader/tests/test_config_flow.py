...
from custom_components.amber_express_trader.config_flow import validate_api_token
...
async def test_full_flow_single_site(hass: HomeAssistant, mock_amber_api: MagicMock) -> None:
    ...
    with (
        patch(
            "custom_components.amber_express_trader.async_setup_entry",
            return_value=True,
        ),
        patch(
            "custom_components.amber_express_trader.async_unload_entry",
            return_value=True,
        ),
    ):
        result = await hass.config_entries.flow.async_configure(
...
async def test_full_flow_single_site_sets_default_forecast_intervals(
    ...
    with (
        patch("custom_components.amber_express_trader.async_setup_entry", return_value=True),
        patch("custom_components.amber_express_trader.async_unload_entry", return_value=True),
    ):
...
async def test_full_flow_single_site_sets_trading_and_polling_defaults(
    hass: HomeAssistant, mock_amber_api: MagicMock
) -> None:
    """Test new subentries default trading and polling options are stored."""
    result = await hass.config_entries.flow.async_init(DOMAIN, context={"source": config_entries.SOURCE_USER})
    result = await hass.config_entries.flow.async_configure(result["flow_id"], {CONF_API_TOKEN: "valid_token"})
    ...
    with (
        ...
        entry = result["result"]
        subentry = next(iter(entry.subentries.values()))
        assert subentry.data[CONF_POLLING_STRATEGY] == DEFAULT_POLLING_STRATEGY
        assert subentry.data[CONF_FIXED_BOUNDARY_OFFSETS] == DEFAULT_FIXED_BOUNDARY_OFFSETS
        assert subentry.data[CONF_ZERO_PRICE_DEADBAND] == DEFAULT_ZERO_PRICE_DEADBAND
        assert subentry.data[CONF_EXPORT_PRICE_FLOOR] == DEFAULT_EXPORT_PRICE_FLOOR
        assert subentry.data[CONF_CHARGE_PRICE_CEILING] == DEFAULT_CHARGE_PRICE_CEILING
        assert subentry.data[CONF_SPIKE_PRICE_THRESHOLD] == DEFAULT_SPIKE_PRICE_THRESHOLD
        assert subentry.data[CONF_TARGET_GRID_BUY_KWH] == DEFAULT_TARGET_GRID_BUY_KWH
        assert subentry.data[CONF_BATTERY_SOC_ENTITY] == DEFAULT_SITE_CONTEXT_ENTITY
        assert subentry.data[CONF_BATTERY_POWER_ENTITY] == DEFAULT_SITE_CONTEXT_ENTITY
        assert subentry.data[CONF_GRID_POWER_ENTITY] == DEFAULT_SITE_CONTEXT_ENTITY
        assert subentry.data[CONF_SOLAR_POWER_ENTITY] == DEFAULT_SITE_CONTEXT_ENTITY
        assert subentry.data[CONF_HOUSE_LOAD_ENTITY] == DEFAULT_SITE_CONTEXT_ENTITY
        assert subentry.data[CONF_PV_ENERGY_TODAY_ENTITY] == DEFAULT_SITE_CONTEXT_ENTITY
        assert subentry.data[CONF_PV_FORECAST_REMAINING_TODAY_ENTITY] == DEFAULT_SITE_CONTEXT_ENTITY
        assert subentry.data[CONF_BATTERY_CHARGE_ENERGY_TODAY_ENTITY] == DEFAULT_SITE_CONTEXT_ENTITY
        assert subentry.data[CONF_BATTERY_DISCHARGE_ENERGY_TODAY_ENTITY] == DEFAULT_SITE_CONTEXT_ENTITY
        assert subentry.data[CONF_BATTERY_USABLE_KWH] == DEFAULT_SITE_CONTEXT_VALUE
        assert subentry.data[CONF_BATTERY_MIN_RESERVE_KWH] == DEFAULT_SITE_CONTEXT_VALUE
        assert subentry.data[CONF_INVERTER_MAX_CHARGE_KW] == DEFAULT_SITE_CONTEXT_VALUE
        assert subentry.data[CONF_INVERTER_MAX_DISCHARGE_KW] == DEFAULT_SITE_CONTEXT_VALUE
        assert subentry.data[CONF_NORMAL_EXPORT_LIMIT_KW] == DEFAULT_SITE_CONTEXT_VALUE
        assert subentry.data[CONF_ALLOW_GRID_CHARGE] == DEFAULT_ALLOW_GRID_CHARGE
        assert subentry.data[CONF_ALLOW_BATTERY_EXPORT] == DEFAULT_ALLOW_BATTERY_EXPORT
        assert len(SITE_CONTEXT_ENTITY_OPTIONS) == 9
