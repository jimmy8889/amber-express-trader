...
class SmartPollingManager:
    ...
    def __init__(
        ...
        *,
        polling_strategy: str = DEFAULT_POLLING_STRATEGY,
        fixed_boundary_offsets: str = DEFAULT_FIXED_BOUNDARY_OFFSETS,
    ) -> None:
        ...
        Args:
            ...
            polling_strategy: Polling strategy name.
            fixed_boundary_offsets: Comma-separated fixed poll offsets in seconds.
        ...
        self._fixed_boundary_offsets = parse_fixed_boundary_offsets(fixed_boundary_offsets)
    ...
    def should_poll(self, *, has_data: bool) -> bool:
        ...
        if self._polling_strategy in (POLLING_STRATEGY_FIXED_BOUNDARY, POLLING_STRATEGY_HYBRID):
            next_fixed_offset = self._get_next_fixed_offset(elapsed)
            if next_fixed_offset is not None:
                return elapsed >= next_fixed_offset
            if self._polling_strategy == POLLING_STRATEGY_FIXED_BOUNDARY:
                return False
    ...
    def get_next_poll_delay(self) -> float | None:
        ...
        if self._polling_strategy in (POLLING_STRATEGY_FIXED_BOUNDARY, POLLING_STRATEGY_HYBRID):
            next_fixed_offset = self._get_next_fixed_offset(elapsed)
            if next_fixed_offset is not None:
                return max(0, next_fixed_offset - elapsed)
            if self._polling_strategy == POLLING_STRATEGY_FIXED_BOUNDARY:
                return None
    ...
    def get_next_poll_time(self) -> datetime | None:
        ...
        elapsed = (datetime.now(UTC) - self._current_interval_start).total_seconds()
        next_poll_seconds = None
        if self._polling_strategy in (POLLING_STRATEGY_FIXED_BOUNDARY, POLLING_STRATEGY_HYBRID):
            next_poll_seconds = self._get_next_fixed_offset(elapsed)
            if next_poll_seconds is None and self._polling_strategy == POLLING_STRATEGY_FIXED_BOUNDARY:
                return self._current_interval_start + timedelta(minutes=self._interval_length)
    ...
    def _get_next_fixed_offset(self, elapsed: float) -> float | None:
        """Return the next fixed offset after the current poll count."""
        fixed_poll_index = max(0, self._poll_count_this_interval - 1)
        for offset in self._fixed_boundary_offsets[fixed_poll_index:]:
            if elapsed <= offset:
                return offset
        return None
    ...
    def get_cdf_stats(self) -> CDFPollingStats:
        ...
        stats = self._cdf_strategy.get_stats()
        if self._polling_strategy == POLLING_STRATEGY_ADAPTIVE:
            return stats
        scheduled_polls = list(self._fixed_boundary_offsets)
        if self._polling_strategy == POLLING_STRATEGY_HYBRID:
            scheduled_polls.extend(t for t in stats.scheduled_polls if t not in scheduled_polls)
        return CDFPollingStats(
            observation_count=stats.observation_count,
            scheduled_polls=scheduled_polls,
            next_poll_index=stats.next_poll_index,
            confirmatory_poll_count=stats.confirmatory_poll_count,
            polls_per_interval=stats.polls_per_interval,
            last_observation=stats.last_observation,
        )
...
def parse_fixed_boundary_offsets(value: str | list[int] | list[float] | tuple[int | float, ...]) -> list[float]:
    """Parse configured fixed boundary offsets."""
    raw_offsets = [part.strip() for part in value.split(",")] if isinstance(value, str) else list(value)
