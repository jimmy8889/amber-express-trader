...
from custom_components.amber_express_trader.polling import (
    IntervalObservation,
    PollingState,
    SmartPollingManager,
...
class TestFixedBoundaryOffsets:
    """Tests for fixed boundary offset parsing."""

    def test_parses_sorted_unique_offsets(self) -> None:
        """Offsets are parsed, sorted, and deduplicated."""
        assert parse_fixed_boundary_offsets("15, 3,8,3,bad,,20") == [3.0, 8.0, 15.0, 20.0]

    def test_ignores_negative_offsets(self) -> None:
        """Negative offsets are ignored."""
        assert parse_fixed_boundary_offsets("-1,0,5") == [0.0, 5.0]


class TestPollingStrategySelection:
    """Tests for polling strategy selection."""

    def test_adaptive_uses_cdf_delay(self) -> None:
        """Adaptive strategy keeps the CDF schedule."""
        manager = SmartPollingManager(
            5,
            get_cold_start_observations(),
            polling_strategy=POLLING_STRATEGY_ADAPTIVE,
        )

        with (
            patch("custom_components.amber_express_trader.polling.smart_polling.datetime") as mock_datetime,
            patch("custom_components.amber_express_trader.polling.cdf_polling.datetime") as mock_cdf_datetime,
        ):
            ...
            assert manager.get_next_poll_delay() == 59.0

    def test_fixed_boundary_uses_configured_offsets(self) -> None:
        """Fixed strategy schedules configured offsets before stopping."""
        manager = SmartPollingManager(
            5,
            get_cold_start_observations(),
            polling_strategy=POLLING_STRATEGY_FIXED_BOUNDARY,
            fixed_boundary_offsets="3,8",
        )

        with patch("custom_components.amber_express_trader.polling.smart_polling.datetime") as mock_datetime:
            base_time = datetime(2024, 1, 1, 10, 0, 0, tzinfo=UTC)
            mock_datetime.now.return_value = base_time
            manager.should_poll(has_data=True)
            ...
            assert manager.get_next_poll_delay() == 2.0

            manager.on_poll_started()
            mock_datetime.now.return_value = base_time + timedelta(seconds=4)
            assert manager.get_next_poll_delay() == 4.0

    def test_fixed_boundary_skips_missed_offsets(self) -> None:
        """Fixed strategy does not replay offsets that already passed."""
        manager = SmartPollingManager(
            5,
            get_cold_start_observations(),
            polling_strategy=POLLING_STRATEGY_FIXED_BOUNDARY,
        ...
        with patch("custom_components.amber_express_trader.polling.smart_polling.datetime") as mock_datetime:
            ...
            assert manager.get_next_poll_delay() is None

    def test_hybrid_falls_back_to_adaptive_after_fixed_offsets(self) -> None:
        """Hybrid uses fixed offsets first, then the adaptive CDF chain."""
        manager = SmartPollingManager(
            5,
            get_cold_start_observations(),
            polling_strategy=POLLING_STRATEGY_HYBRID,
            fixed_boundary_offsets="3",
        )
        ...
        with (
            ...
            manager.on_poll_started()
            assert manager.get_next_poll_delay() == 3.0

            manager.on_poll_started()
            mock_datetime.now.return_value = base_time + timedelta(seconds=4)
            assert manager.get_next_poll_delay() == 56.0
